...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        console.log('✅ News query successful, found:', news?.length || 0);
        ...
        const response: ApiResponse<{ news: News[], total: number, page: number }> = {
            success: true,
            data: {
                news: news || [],
                total,
...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        if (!body.author || !body.title || !body.text || !body.tag) {
            return NextResponse.json(
                {
                    success: false,
        ...
        if (body.text.length > 5000) {
            return NextResponse.json(
                {
                    success: false,
                    error: 'Текст новости не должен превышать 5000 символов'
                },
                { status: 400 }
            );
        }
        ...
        const [result] = await pool.execute(
            `INSERT INTO news (author, title, text, date, time, tag)
             VALUES (?, ?, ?, ?, ?, ?)`,
            [
                body.author,
                body.title,
...
function generateVerificationEmailHtml(username: string, link: string): string {
    ...
    <head>
        ...
        <title>Новая новость - ФНБТС</title>
        <style>
            body {
        ...
        <div class="container">
            ...
            <div style="margin-bottom: 25px;">
                ...
                <p style="font-size: 14px; line-height: 1.5; margin: 0 0 15px 0; color: #f4f4f4;">
                    На ФНБТС вышла новая новость! Для её просмотра нажмите на кнопку или перейдите по ссылке ниже:
                </p>
            </div>
